...
- **[per-tenant-fine-tuning.md](./per-tenant-fine-tuning.md)** — When per-tenant fine-tuning is the right pattern (very few cases), when adapters / LoRAs make it cheaper, and the operational cost (per-tenant model lifecycle, per-tenant eval, per-tenant rollback) that usually steers regulated workloads back toward strong RAG + shared base model.
- **[data-residency-patterns.md](./data-residency-patterns.md)** — Region-pinning at the infrastructure layer (provider region, model deployment region, data store region), the per-region model-availability matrix that constrains the design, and the architecture for tenants whose data may not cross specific borders. Includes the GovCloud / sovereign-cloud variant and the FedRAMP boundary pattern.
